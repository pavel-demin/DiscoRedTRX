...
ADC2s_t ADC2s[] = {
  ...
  {ADC_PA4 ,GPIOA,GPIO_PIN_4,ADC_CHANNEL_4,MW_32},   // ADC an PA4 = ADC123_IN4
  {ADC_PA6 ,GPIOA,GPIO_PIN_6,ADC_CHANNEL_6,MW_32},   // ADC an PA6 = ADC123_IN6
};
...
uint16_t UB_ADC2_SINGLE_Read(ADC2s_NAME_t adc_name)
{
  ...
  sConfig.Offset       = 0;
  // Messung starten
  if (HAL_ADC_ConfigChannel(&Adc2Handle, &sConfig) != HAL_OK) return 0;
  if (HAL_ADC_Start(&Adc2Handle) != HAL_OK) return 0;
  // warte bis Messung fertig ist
  HAL_ADC_PollForConversion(&Adc2Handle, 10);
  if(HAL_IS_BIT_CLR(HAL_ADC_GetState(&Adc2Handle), HAL_ADC_STATE_REG_EOC)) return 0;
  // Messwert auslesen
