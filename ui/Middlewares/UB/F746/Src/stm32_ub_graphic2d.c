...
void UB_Graphic2D_ClearSreenDMA(uint16_t c) {
  ...
  if (HAL_DMA2D_Init(&hDma2dHandler) == HAL_OK) {
    if (HAL_DMA2D_ConfigLayer(&hDma2dHandler, LCD_CurrentLayer) == HAL_OK) {
      if (HAL_DMA2D_Start(&hDma2dHandler, col32,
                          (uint32_t)LCD_CurrentFrameBuffer, LCD_MAXX,
                          LCD_MAXY) == HAL_OK) {
...
//--------------------------------------------------------------
void UB_Graphic2D_DrawLineNormal(uint16_t x1, uint16_t y1, uint16_t x2,
...
//--------------------------------------------------------------
void UB_Graphic2D_DrawStraightDMA(uint16_t xp, uint16_t yp, uint16_t l,
                                  LCD_DIR_t d, uint16_t c) {
  ...
  if (HAL_DMA2D_Init(&hDma2dHandler) == HAL_OK) {
    if (HAL_DMA2D_ConfigLayer(&hDma2dHandler, LCD_CurrentLayer) == HAL_OK) {
      if (HAL_DMA2D_Start(&hDma2dHandler, col32, (uint32_t)Xaddress, w, h) ==
          HAL_OK) {
        HAL_DMA2D_PollForTransfer(&hDma2dHandler, 10);
      }
    }
...
// Hoehe  = h
//--------------------------------------------------------------
void UB_Graphic2D_DrawRectDMA(uint16_t xp, uint16_t yp, uint16_t w, uint16_t h,
                              uint16_t c) {
  int16_t d;
...
// Hoehe  = h
//--------------------------------------------------------------
void UB_Graphic2D_DrawFullRectDMA(uint16_t xp, uint16_t yp, uint16_t w,
...
// (benutzt wird der Bresenham-Algorithmus)
//--------------------------------------------------------------
void UB_Graphic2D_DrawCircleNormal(uint16_t xp, uint16_t yp, uint16_t r,
                                   uint16_t c) {
...
// Radius = r
//--------------------------------------------------------------
void UB_Graphic2D_DrawFullCircleDMA(uint16_t xp, uint16_t yp, uint16_t r,
                                    uint16_t c) {
  int32_t D;
  uint32_t CurX;
  uint32_t CurY;
...
void UB_Graphic2D_CopyImgDMA(UB_Image *img, DMA2D_Koord koord) {
  ...
  dest_address =
      LCD_CurrentFrameBuffer + 2 * (LCD_MAXX * koord.dest_yp + koord.dest_xp);
  ...
  if (koord.source_h == 0)
    return;
  if (koord.source_xp + koord.source_w > picture_width)
    return;
  if (koord.source_yp + koord.source_h > picture_height)
    return;
  if (koord.dest_xp + koord.source_w > LCD_MAXX)
    return;
  if (koord.dest_yp + koord.source_h > LCD_MAXY)
    return;
  ...
  hDma2dHandler.LayerCfg[LCD_CurrentLayer].InputOffset =
      picture_width - koord.source_w;
  hDma2dHandler.Instance = DMA2D;

  // DMA2D Initialization
  if (HAL_DMA2D_Init(&hDma2dHandler) == HAL_OK) {
    if (HAL_DMA2D_ConfigLayer(&hDma2dHandler, LCD_CurrentLayer) == HAL_OK) {
      if (HAL_DMA2D_Start(&hDma2dHandler, source_address, dest_address, w, h) ==
          HAL_OK) {
        HAL_DMA2D_PollForTransfer(&hDma2dHandler, 10);
      }
    }
...
//   -1 ,wenn x < 0
//--------------------------------------------------------------
int16_t P_Graphic2D_sgn(int16_t x) { return (x > 0) ? 1 : (x < 0) ? -1 : 0; }
