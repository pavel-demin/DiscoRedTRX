...
int16_t UB_I2C3_ReadByte(uint8_t slave_adr, uint8_t adr) {
  ...
  // adresse senden
  if (HAL_I2C_Master_Transmit(&I2c3Handle, (uint16_t)slave_adr,
                              (uint8_t *)I2C3_DATA, 1,
                              I2C3_TIMEOUT) != HAL_OK) {
    ret_wert = -1;
    if (HAL_I2C_GetError(&I2c3Handle) != HAL_I2C_ERROR_AF)
      ret_wert = -2;
  }
  if (ret_wert != 0)
    return (ret_wert);
  ...
  // daten lesen
  if (HAL_I2C_Master_Receive(&I2c3Handle, (uint16_t)slave_adr,
                             (uint8_t *)I2C3_DATA, 1, I2C3_TIMEOUT) != HAL_OK) {
    ret_wert = -3;
    if (HAL_I2C_GetError(&I2c3Handle) != HAL_I2C_ERROR_AF)
      ret_wert = -4;
  }
