...
static uint8_t P_QSPI_AutoPollingMemReady(QSPI_HandleTypeDef *hqspi, uint32_t Timeout);
...
uint8_t UB_QFlash_Init(void)
{
  ...
  // reset
  if(P_QSPI_ResetMemory(&QSPIHandle) != QSPI_OK) return QSPI_NOT_SUPPORTED; 

  // config
  if(P_QSPI_DummyCyclesCfg(&QSPIHandle) != QSPI_OK) return QSPI_NOT_SUPPORTED;
...
uint8_t UB_QFlash_Erase_Complete(void)
{
  ...
  if(P_QSPI_WriteEnable(&QSPIHandle) != QSPI_OK) return QSPI_ERROR;

  // Send the command
  if(HAL_QSPI_Command(&QSPIHandle, &s_command, HAL_QPSI_TIMEOUT_DEFAULT_VALUE) != HAL_OK) return QSPI_ERROR;
  
  // Configure automatic polling mode to wait for end of erase
  if(P_QSPI_AutoPollingMemReady(&QSPIHandle, N25Q128A_BULK_ERASE_MAX_TIME) != QSPI_OK) return QSPI_ERROR;
...
uint8_t UB_QFlash_Erase_SubSector(uint32_t subsector_nr)
{
  ...
  if(subsector_nr>=N25Q128A_SUBSECTOR_CNT) return QSPI_ERROR;
  ...
  // Configure automatic polling mode to wait for end of erase
  if(P_QSPI_AutoPollingMemReady(&QSPIHandle, N25Q128A_SUBSECTOR_ERASE_MAX_TIME) != QSPI_OK) return QSPI_ERROR;
...
uint8_t UB_QFlash_Read_Block8b(uint32_t start_adr, uint32_t size, uint8_t* data_buf)
{
  ...
  if(start_adr>=N25Q128A_FLASH_SIZE) return QSPI_ERROR;
  if((start_adr+size)>=N25Q128A_FLASH_SIZE) return QSPI_ERROR;
  ...
  if(HAL_QSPI_Receive(&QSPIHandle, data_buf, HAL_QPSI_TIMEOUT_DEFAULT_VALUE) != HAL_OK) return QSPI_ERROR;
...
uint8_t UB_QFlash_Write_Block8b(uint32_t start_adr, uint32_t size, uint8_t* data_buf)
{
  ...
  do
  {
    ...
    if(HAL_QSPI_Transmit(&QSPIHandle, data_buf, HAL_QPSI_TIMEOUT_DEFAULT_VALUE) != HAL_OK) return QSPI_ERROR;
    
    // Configure automatic polling mode to wait for end of program
    if(P_QSPI_AutoPollingMemReady(&QSPIHandle, HAL_QPSI_TIMEOUT_DEFAULT_VALUE) != QSPI_OK) return QSPI_ERROR;
...
static uint8_t P_QSPI_ResetMemory(QSPI_HandleTypeDef *hqspi)
{
  ...
  if(P_QSPI_AutoPollingMemReady(hqspi, HAL_QPSI_TIMEOUT_DEFAULT_VALUE) != QSPI_OK) return QSPI_ERROR;
...
static uint8_t P_QSPI_DummyCyclesCfg(QSPI_HandleTypeDef *hqspi)
{
  ...
  // Enable write operations
  if(P_QSPI_WriteEnable(hqspi) != QSPI_OK) return QSPI_ERROR;
...
static uint8_t P_QSPI_AutoPollingMemReady(QSPI_HandleTypeDef *hqspi, uint32_t Timeout)
{
  ...
  if(HAL_QSPI_AutoPolling(hqspi, &s_command, &s_config, Timeout) != HAL_OK) return QSPI_ERROR;
...
static uint8_t P_QSPI_WriteEnable(QSPI_HandleTypeDef *hqspi)
{
  ...
  if(HAL_QSPI_AutoPolling(hqspi, &s_command, &s_config, HAL_QPSI_TIMEOUT_DEFAULT_VALUE) != HAL_OK) return QSPI_ERROR;
