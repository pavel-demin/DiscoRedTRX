...
ErrorStatus UB_SDRAM_Init(void) {
  ...
  if (istwert == 0x5A3C)
    ret_wert = SUCCESS; // RAM vorhanden
...
//--------------------------------------------------------------
ErrorStatus UB_SDRAM_WriteBuffer32b(uint32_t *ptrBuffer, uint32_t startAdr,
                                    uint32_t lenBuffer) {
  if (HAL_SDRAM_Write_32b(&sdramHandle,
                          (uint32_t *)(startAdr + SDRAM_START_ADR), ptrBuffer,
                          lenBuffer) != HAL_OK) {
    return ERROR;
  } else {
    return SUCCESS;
...
//--------------------------------------------------------------
ErrorStatus UB_SDRAM_ReadBuffer32b(uint32_t *ptrBuffer, uint32_t startAdr,
                                   uint32_t lenBuffer) {
  if (HAL_SDRAM_Read_32b(&sdramHandle, (uint32_t *)(startAdr + SDRAM_START_ADR),
                         ptrBuffer, lenBuffer) != HAL_OK) {
    return ERROR;
  } else {
    return SUCCESS;
  }
...
// lenBuffer : anzahl der Daten die geschrieben werden sollen
//--------------------------------------------------------------
ErrorStatus UB_SDRAM_WriteBuffer32bDMA(uint32_t *ptrBuffer, uint32_t startAdr,
                                       uint32_t lenBuffer) {
  if (HAL_SDRAM_Write_DMA(&sdramHandle,
                          (uint32_t *)(startAdr + SDRAM_START_ADR), ptrBuffer,
                          lenBuffer) != HAL_OK) {
    return ERROR;
  } else {
...
ErrorStatus P_SDRAM_Init(void) {
  ...
  /* Timing configuration for 100Mhz as SD clock frequency (System clock is up
   * to 200Mhz) */
  ...
  P_SDRAM_MspInit(
      &sdramHandle,
      NULL); /* __weak function can be rewritten by the application */
