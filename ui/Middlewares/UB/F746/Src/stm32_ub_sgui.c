...
void SGUI_Do(void) {
  ...
  if (UB_SGUI.touch_flag == 1) {
    ...
    touch_aktion = SGUI_ButtonTouch(SGUI.touch.pressed, SGUI.touch.xp,
                                    SGUI.touch.yp, touch_aktion);
#endif

#if SGUI_USE_CHECKBOX == 1
    // check alle Checkboxen auf Touch position
    touch_aktion = SGUI_CheckboxTouch(SGUI.touch.pressed, SGUI.touch.xp,
                                      SGUI.touch.yp, touch_aktion);
#endif

#if SGUI_USE_RADIOBTN == 1
    // check alle RadioButtons auf Touch position
    touch_aktion = SGUI_RadioButtonTouch(SGUI.touch.pressed, SGUI.touch.xp,
                                         SGUI.touch.yp, touch_aktion);
#endif

#if SGUI_USE_LISTBOX == 1
    // check alle Listboxen auf Touch position
    touch_aktion = SGUI_ListboxTouch(SGUI.touch.pressed, SGUI.touch.xp,
                                     SGUI.touch.yp, touch_aktion);
#endif

#if SGUI_USE_SLIDER == 1
    // check alle Slider auf Touch position
    touch_aktion = SGUI_SliderTouch(SGUI.touch.pressed, SGUI.touch.xp,
                                    SGUI.touch.yp, touch_aktion);
#endif

#if SGUI_USE_DROPDOWN == 1
    // check alle Dropdown auf Touch position
    touch_aktion = SGUI_DropdownTouch(SGUI.touch.pressed, SGUI.touch.xp,
                                      SGUI.touch.yp, touch_aktion);
#endif

#if SGUI_USE_SELECTBTN == 1
    // check alle SelectButton auf Touch position
    touch_aktion = SGUI_SelectButtonTouch(SGUI.touch.pressed, SGUI.touch.xp,
                                          SGUI.touch.yp, touch_aktion);
#endif

#if SGUI_USE_INTEDIT == 1
    // check alle IntEdit auf Touch position
    touch_aktion = SGUI_IntEditTouch(SGUI.touch.pressed, SGUI.touch.xp,
                                     SGUI.touch.yp, touch_aktion);
#endif

#if SGUI_USE_FLOATEDIT == 1
    // check alle FloatEdit auf Touch position
    touch_aktion = SGUI_FloatEditTouch(SGUI.touch.pressed, SGUI.touch.xp,
                                       SGUI.touch.yp, touch_aktion);
#endif

#if SGUI_USE_PICTURE == 1
    // check alle Picture auf Touch position
    touch_aktion = SGUI_PictureTouch(SGUI.touch.pressed, SGUI.touch.xp,
                                     SGUI.touch.yp, touch_aktion);
#endif
  }
...
// pause (in ms)
//--------------------------------------------------------------
void SGUI_Pause_ms(uint32_t delay) { HAL_Delay(delay); }

//--------------------------------------------------------------
// auslesen vom Speicherbedarf (in bytes)
//--------------------------------------------------------------
uint32_t SGUI_GetUsedRam(void) { return SGUI_WINDOW.ram_used; }
