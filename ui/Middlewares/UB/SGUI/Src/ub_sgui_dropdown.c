...
SDROPDOWN_t *SGUI_DropdownCreate(uint16_t x, uint16_t y, uint16_t w,
  ...
  if (SGUI_WINDOW.akt_win_ptr == NULL)
    return NULL;
  if (SGUI_DROPDOWN.akt_anz >= (MAX_DROPDOWN_ANZ - 1))
    return NULL;
...
// c3 = Hintergrund
//--------------------------------------------------------------
void SGUI_DropdownSetColor(SDROPDOWN_t *ptr, uint16_t c1, uint16_t c2,
                           uint16_t c3) {
  if (ptr == NULL)
    return;
...
void SGUI_DropdownAddItem(SDROPDOWN_t *ptr, char *txt) {
  ...
  if ((ptr == NULL) || (txt == NULL))
    return;
  if (ptr->item_cnt >= MAX_STRINGLIST_ANZ)
    return;
      ...
      if ((SGUI_DROPDOWN.ptr[n]->status & SDROPDOWN_BIT_EXPAND) == 0) {
        ...
        if (y < y1 + SGUI_DROPDOWN.ptr[n]->height) {
          ...
          if (x > (x2 - SGUI_DROPDOWN.ptr[n]->height)) {
            m = 2;
            if (y > (y1 + SGUI_DROPDOWN.ptr[n]->height +
                     (SGUI_DROPDOWN.ptr[n]->exp_height -
                      SGUI_DROPDOWN.ptr[n]->height) /
                         2)) {
              // pfeil runter
              if (SGUI_DROPDOWN.ptr[n]->item_first <
                  SGUI_DROPDOWN.ptr[n]->item_cnt - 1) {
        ...
        if (m == 1) {
          ...
          if ((SGUI_DROPDOWN.ptr[n]->status & SDROPDOWN_BIT_SLIDER) != 0)
            zeile += SGUI_DROPDOWN.ptr[n]->item_first;
          ...
          if (SGUI_DROPDOWN.ptr[n]->item_cnt > zeile) {
            if (SGUI_DROPDOWN.ptr[n]->item_aktiv != zeile) {
              ...
              P_SGUI_DropdownDraw(SGUI_DROPDOWN.ptr[n]);
              // CallBack aufrufen
              if (SGUI_DROPDOWN.ptr[n]->fkt_ptr != NULL)
                (*SGUI_DROPDOWN.ptr[n]->fkt_ptr)(zeile);
            }
          }
        }
      }
...
void P_SGUI_DropdownDraw(SDROPDOWN_t *ptr) {
  ...
  if ((ptr->status & SDROPDOWN_BIT_EXPAND) != 0) {
    if (cnt > 0) {
      ...
      fy = y + h;
      fx = x + d;
      if (ptr->item_first + cnt > ptr->item_cnt)
        cnt = ptr->item_cnt - ptr->item_first;
      ...
      if ((ptr->status & SDROPDOWN_BIT_SLIDER) != 0) {
        if ((ptr->status & SDROPDOWN_BIT_3DSTYLE) == 0) {
          ...
          sl_size = btn_size + d;
          SGUI_ScreenDraw3DFrame(x + w - sl_size, y + sl_size, sl_size,
                                 px_used + d, STYLE_LOWERED);
        }
        if (px_used > 2 * btn_size) {
          // pfeil-hoch zeichnen
          btn1_yp = y + ptr->height - 1;
          SGUI_ScreenDrawArrowBtn(btn1_xp, btn1_yp, btn_size, SARROW_UP, c2,
                                  c1);
          // pfeil-runter zeichnen
          btn2_xp = btn1_xp;
          btn2_yp = y + ptr->exp_height - d - btn_size + 1;
          SGUI_ScreenDrawArrowBtn(btn2_xp, btn2_yp, btn_size, SARROW_DOWN, c2,
                                  c1);
          if ((ptr->status & SDROPDOWN_BIT_3DSTYLE) != 0) {
            SGUI_ScreenDraw3DFrame(btn1_xp, btn1_yp, btn_size, btn_size,
                                   STYLE_RAISED);
