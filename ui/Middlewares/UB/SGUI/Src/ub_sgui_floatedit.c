...
SFLOATEDIT_t *SGUI_FloatEditCreate(uint16_t x, uint16_t y, uint16_t w,
  ...
  if (SGUI_WINDOW.aktiv_window == 0)
    return NULL;
  if (SGUI_WINDOW.akt_win_ptr == NULL)
    return NULL;
  if (SGUI_FLOATEDIT.akt_anz >= (MAX_FLOATEDIT_ANZ - 1))
    return NULL;
...
//--------------------------------------------------------------
void SGUI_FloatEditSetNumFormat(SFLOATEDIT_t *ptr, uint8_t digits,
                                bool padding) {
  if (ptr == NULL)
    return;
...
// align : [SFLOATEDIT_ALIGN_CENTER, SFLOATEDIT_ALIGN_LEFT,
// SFLOATEDIT_ALIGN_RIGHT]
//--------------------------------------------------------------
...
// c3 = Hintergrund
//--------------------------------------------------------------
void SGUI_FloatEditSetColor(SFLOATEDIT_t *ptr, uint16_t c1, uint16_t c2,
                            uint16_t c3) {
  if (ptr == NULL)
    return;
...
void SGUI_FloatEditSetValue(SFLOATEDIT_t *ptr, float value) {
  if (ptr == NULL)
    return;
  if (value < ptr->f_minvalue)
    value = ptr->f_minvalue;
  if (value > ptr->f_maxvalue)
    value = ptr->f_maxvalue;
...
bool SGUI_FloatEditTouch(bool pressed, uint16_t x, uint16_t y, bool blocked) {
  ...
  if (pressed == false) {
    ...
    if (m == 1) {
      if ((SGUI_FLOATEDIT.ptr[n]->typ == SFLOATEDIT_H) ||
          (SGUI_FLOATEDIT.ptr[n]->typ == SFLOATEDIT_H2)) {
        if (x > (x1 + SGUI_FLOATEDIT.ptr[n]->width / 2)) {
          // pfeil hoch
          SGUI_FLOATEDIT.ptr[n]->f_value += SGUI_FLOATEDIT.ptr[n]->step_value;
          if (SGUI_FLOATEDIT.ptr[n]->f_value >
              SGUI_FLOATEDIT.ptr[n]->f_maxvalue) {
            SGUI_FLOATEDIT.ptr[n]->f_value = SGUI_FLOATEDIT.ptr[n]->f_maxvalue;
          }
          // neu zeichnen
          P_SGUI_FloatEditDraw(SGUI_FLOATEDIT.ptr[n]);
          // CallBack aufrufen
          if (SGUI_FLOATEDIT.ptr[n]->fkt_ptr != NULL)
            (*SGUI_FLOATEDIT.ptr[n]->fkt_ptr)(SGUI_FLOATEDIT.ptr[n]->f_value);
        } else {
          // pfeil runter
          SGUI_FLOATEDIT.ptr[n]->f_value -= SGUI_FLOATEDIT.ptr[n]->step_value;
          if (SGUI_FLOATEDIT.ptr[n]->f_value <
              SGUI_FLOATEDIT.ptr[n]->f_minvalue) {
...
void SGUI_FloatEditUpdate(void) {
  ...
  for (n = 0; n < SGUI_FLOATEDIT.akt_anz; n++) {
    ...
    if (SGUI_WINDOW.aktiv_window == SGUI_FLOATEDIT.ptr[n]->window_nr) {
      ...
      if ((status != 0) || (SGUI_WINDOW.update_all == true))
        P_SGUI_FloatEditDraw(SGUI_FLOATEDIT.ptr[n]);
    }
  }
}
...
void P_SGUI_FloatEditDraw(SFLOATEDIT_t *ptr) {
  ...
  if ((ptr->status & SFLOATEDIT_BIT_ARROW) != 0) {
    ...
    if ((ptr->typ == SFLOATEDIT_H) || (ptr->typ == SFLOATEDIT_H2)) {
      ...
      if (ptr->typ == SFLOATEDIT_H) {
        ...
        SGUI_ScreenDrawArrowBtn(btn1_xp, btn1_yp, btn_size, SARROW_DOWN, c2,
                                c1);
        // pfeil-hoch zeichnen
        SGUI_ScreenDrawArrowBtn(btn2_xp, btn2_yp, btn_size, SARROW_UP, c2, c1);
      } else {
        // pfeil-links zeichnen
        SGUI_ScreenDrawArrowBtn(btn1_xp, btn1_yp, btn_size, SARROW_LEFT, c2,
                                c1);
        // pfeil-rechts zeichnen
        SGUI_ScreenDrawArrowBtn(btn2_xp, btn2_yp, btn_size, SARROW_RIGHT, c2,
                                c1);
      }
      if ((ptr->status & SFLOATEDIT_BIT_3DSTYLE) != 0) {
        SGUI_ScreenDraw3DFrame(btn1_xp, btn1_yp, btn_size, btn_size,
                               STYLE_RAISED);
        SGUI_ScreenDraw3DFrame(btn2_xp, btn2_yp, btn_size, btn_size,
                               STYLE_RAISED);
      }
  ...
  fx = x;
  if (w > px_used) {
    if (ptr->align == SFLOATEDIT_ALIGN_CENTER)
      fx = ((w - px_used) / 2) + x;
    if (ptr->align == SFLOATEDIT_ALIGN_LEFT)
      fx = x + xd;
    if (ptr->align == SFLOATEDIT_ALIGN_RIGHT)
      fx = x + (w - px_used) - xd;
  }
