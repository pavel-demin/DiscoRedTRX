...
SGRAPH_t *SGUI_GraphCreate(uint16_t x, uint16_t y, uint16_t w, uint16_t h) {
  ...
  if (SGUI_WINDOW.akt_win_ptr == NULL)
    return NULL;
  if (SGUI_GRAPH.akt_anz >= (MAX_GRAPH_ANZ))
    return NULL; // ** WK (war ... -1)
...
void SGUI_GraphCreateDataArray(SGRAPH_t *ptr, uint8_t ch, uint16_t cnt) {
  ...
  if (ptr == NULL)
    return;
  if ((ch == 0) || (ch > MAX_GRAPHCH_ANZ))
    return;
  if ((cnt == 0) || (cnt > MAX_GRAPHDATA_ANZ))
    return;
...
//--------------------------------------------------------------
void SGUI_GraphSetCHValue(SGRAPH_t *ptr, uint8_t ch, uint16_t pos,
                          uint8_t value) {
  if (ptr == NULL)
    return;
  if (ch >= ptr->ch_cnt)
  ...
  for (n = 0; n < SGUI_GRAPH.akt_anz; n++) {
    ...
    if (SGUI_WINDOW.aktiv_window == SGUI_GRAPH.ptr[n]->window_nr) {
      ...
      if ((status != 0) || (SGUI_WINDOW.update_all == true))
        P_SGUI_GraphDraw(SGUI_GRAPH.ptr[n]);
    }
  }
}
...
DataArray_t *P_SGUI_GraphCreateDataArray(uint8_t ch, uint16_t cnt) {
  ...
  if (ch == 0)
    return NULL;
  if (ch >= MAX_GRAPHCH_ANZ)
    return NULL;
  if (cnt >= MAX_GRAPHDATA_ANZ)
    return NULL;
...
void SGUI_GraphWriteColumn(SGRAPH_t *ptr, uint8_t ch, uint16_t pos,
                           uint8_t wert) // ** WK bar graphic
{
