...
SINTEDIT_t *SGUI_IntEditCreate(uint16_t x, uint16_t y, uint16_t w, uint16_t h) {
  ...
  if (SGUI_WINDOW.akt_win_ptr == NULL)
    return NULL;
  if (SGUI_INTEDIT.akt_anz >= (MAX_INTEDIT_ANZ - 1))
    return NULL;
...
// c3 = Hintergrund
//--------------------------------------------------------------
void SGUI_IntEditSetColor(SINTEDIT_t *ptr, uint16_t c1, uint16_t c2,
                          uint16_t c3) {
  if (ptr == NULL)
    return;
...
void SGUI_IntEditSetMinMax(SINTEDIT_t *ptr, int32_t min, int32_t max) {
  ...
  if (ptr->i32_value < min)
    ptr->i32_value = min;
  if (ptr->i32_value > max)
    ptr->i32_value = max;
}
...
void SGUI_IntEditSetValue(SINTEDIT_t *ptr, int32_t value) {
  if (ptr == NULL)
    return;
  if (value < ptr->i32_minvalue)
    value = ptr->i32_minvalue;
  if (value > ptr->i32_maxvalue)
    value = ptr->i32_maxvalue;
...
bool SGUI_IntEditTouch(bool pressed, uint16_t x, uint16_t y, bool blocked) {
  ...
  if (pressed == false) {
    ...
    if (m == 1) {
      if ((SGUI_INTEDIT.ptr[n]->typ == SINTEDIT_H) ||
          (SGUI_INTEDIT.ptr[n]->typ == SINTEDIT_H2)) {
        if (x > (x1 + SGUI_INTEDIT.ptr[n]->width / 2)) {
          // pfeil hoch
          SGUI_INTEDIT.ptr[n]->i32_value += SGUI_INTEDIT.ptr[n]->step_value;
          if (SGUI_INTEDIT.ptr[n]->i32_value >
              SGUI_INTEDIT.ptr[n]->i32_maxvalue) {
            SGUI_INTEDIT.ptr[n]->i32_value = SGUI_INTEDIT.ptr[n]->i32_maxvalue;
          }
          // neu zeichnen
          P_SGUI_IntEditDraw(SGUI_INTEDIT.ptr[n]);
          // CallBack aufrufen
          if (SGUI_INTEDIT.ptr[n]->fkt_ptr != NULL)
            (*SGUI_INTEDIT.ptr[n]->fkt_ptr)(SGUI_INTEDIT.ptr[n]->i32_value);
        } else {
          // pfeil runter
          SGUI_INTEDIT.ptr[n]->i32_value -= SGUI_INTEDIT.ptr[n]->step_value;
          if (SGUI_INTEDIT.ptr[n]->i32_value <
              SGUI_INTEDIT.ptr[n]->i32_minvalue) {
...
void P_SGUI_IntEditDraw(SINTEDIT_t *ptr) {
  ...
  fx = x;
  if (w > px_used) {
    if (ptr->align == SINTEDIT_ALIGN_CENTER)
      fx = ((w - px_used) / 2) + x;
    if (ptr->align == SINTEDIT_ALIGN_LEFT)
      fx = x + xd;
    if (ptr->align == SINTEDIT_ALIGN_RIGHT)
      fx = x + (w - px_used) - xd;
  }
