...
SLABEL_t *SGUI_LabelCreate(uint16_t x, uint16_t y, uint16_t w, uint16_t h) {
  ...
  if (SGUI_WINDOW.akt_win_ptr == NULL)
    return NULL;
  if (SGUI_LABEL.akt_anz >= (MAX_LABEL_ANZ - 1))
    return NULL;
  ...
  for (n = 0; n < SGUI_LABEL.akt_anz; n++) {
    ...
    if (SGUI_WINDOW.aktiv_window == SGUI_LABEL.ptr[n]->window_nr) {
      ...
      if ((status != 0) || (SGUI_WINDOW.update_all == true))
        P_SGUI_LabelDraw(SGUI_LABEL.ptr[n]);
    }
  }
}
...
void P_SGUI_LabelDraw(SLABEL_t *ptr) {
  ...
  fx = x;
  if (w > px_used) {
    if (ptr->align == SLABEL_ALIGN_CENTER)
      fx = ((w - px_used) / 2) + x;
    if (ptr->align == SLABEL_ALIGN_LEFT)
      fx = x + d;
    if (ptr->align == SLABEL_ALIGN_RIGHT)
      fx = x + (w - px_used) - d;
  }
