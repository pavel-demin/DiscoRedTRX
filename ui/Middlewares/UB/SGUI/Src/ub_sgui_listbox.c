...
SLISTBOX_t *SGUI_ListboxCreate(uint16_t x, uint16_t y, uint16_t w, uint16_t h) {
  ...
  if (SGUI_WINDOW.akt_win_ptr == NULL)
    return NULL;
  if (SGUI_LISTBOX.akt_anz >= (MAX_LISTBOX_ANZ - 1))
    return NULL;
...
// c3 = Hintergrund
//--------------------------------------------------------------
void SGUI_ListboxSetColor(SLISTBOX_t *ptr, uint16_t c1, uint16_t c2,
                          uint16_t c3) {
  if (ptr == NULL)
    return;
      ...
      if ((SGUI_LISTBOX.ptr[n]->status & SLISTBOX_BIT_SLIDER) != 0) {
        if (x > (x2 - SGUI_LISTBOX.ptr[n]->slider_size)) {
          ...
          if (y > (y1 + SGUI_LISTBOX.ptr[n]->height / 2)) {
            // pfeil runter
            if (SGUI_LISTBOX.ptr[n]->item_first <
                SGUI_LISTBOX.ptr[n]->item_cnt - 1) {
      ...
      if (m == 1) {
        ...
        if (SGUI_LISTBOX.ptr[n]->item_cnt > zeile) {
          if (SGUI_LISTBOX.ptr[n]->item_aktiv != zeile) {
            ...
            P_SGUI_ListboxDraw(SGUI_LISTBOX.ptr[n]);
            // CallBack aufrufen
            if (SGUI_LISTBOX.ptr[n]->fkt_ptr != NULL)
              (*SGUI_LISTBOX.ptr[n]->fkt_ptr)(zeile);
          }
        }
      }
    }
...
void P_SGUI_ListboxDraw(SLISTBOX_t *ptr) {
  ...
  if ((ptr->status & SLISTBOX_BIT_SLIDER) != 0) {
    ...
    if ((ptr->status & SLISTBOX_BIT_3DSTYLE) != 0) {
      ...
      // linie zeichnen
      SGUI_ScreenDrawLine(x + w - sl_size - d + 1, y, x + w - sl_size - d + 1,
                          y + h - 1, c2);
      btn_size = sl_size;
    }
