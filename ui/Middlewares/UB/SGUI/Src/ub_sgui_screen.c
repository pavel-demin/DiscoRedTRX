...
// interne Funktionen
//--------------------------------------------------------------
void P_SGUI_Screen3DArc(uint16_t xp, uint16_t yp, uint16_t r, uint16_t c,
                        uint8_t m);
...
void SGUI_ScreenDrawPixel(uint16_t xp, uint16_t yp, uint16_t c) {
  // check auf limit
  if (xp >= SGUI_SCREEN_MAXX)
    xp = SGUI_SCREEN_MAXX - 1;
  if (yp >= SGUI_SCREEN_MAXY)
    yp = SGUI_SCREEN_MAXY - 1;
...
// Farbe = c
//--------------------------------------------------------------
void SGUI_ScreenDrawLine(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2,
                         uint16_t c) {
  // check auf limit
  if (x1 >= SGUI_SCREEN_MAXX)
    x1 = SGUI_SCREEN_MAXX - 1;
  if (y1 >= SGUI_SCREEN_MAXY)
    y1 = SGUI_SCREEN_MAXY - 1;
  if (x2 >= SGUI_SCREEN_MAXX)
    x2 = SGUI_SCREEN_MAXX - 1;
  if (y2 >= SGUI_SCREEN_MAXY)
    y2 = SGUI_SCREEN_MAXY - 1;
...
// Farbe  = c
//--------------------------------------------------------------
void SGUI_ScreenDrawFullRect(uint16_t xp, uint16_t yp, uint16_t w, uint16_t h,
                             uint16_t c) {
  // check auf limit
  if (xp >= SGUI_SCREEN_MAXX)
    xp = SGUI_SCREEN_MAXX - 1;
  ...
  if ((w == 0) || (h == 0))
    return;
  if ((xp + w) > SGUI_SCREEN_MAXX)
    w = SGUI_SCREEN_MAXX - xp;
  if ((yp + h) > SGUI_SCREEN_MAXY)
    h = SGUI_SCREEN_MAXY - yp;
...
// Radius = r
//--------------------------------------------------------------
void SGUI_ScreenDrawFullCircle(uint16_t xp, uint16_t yp, uint16_t r,
                               uint16_t c) {
  // check auf limit
  if (xp >= SGUI_SCREEN_MAXX)
    xp = SGUI_SCREEN_MAXX - 1;
  ...
  if (r == 0)
    return;
  if ((xp + r) > SGUI_SCREEN_MAXX)
    r = SGUI_SCREEN_MAXX - xp;
  if ((yp + r) > SGUI_SCREEN_MAXY)
    r = SGUI_SCREEN_MAXY - yp;
...
// Farbe = c
//--------------------------------------------------------------
void SGUI_ScreenStraight(uint16_t xp, uint16_t yp, uint16_t l, LCD_DIR_t d,
                         uint16_t c) {
  // check auf limit
  if (xp >= SGUI_SCREEN_MAXX)
    xp = SGUI_SCREEN_MAXX - 1;
  ...
  if (d == LCD_DIR_HORIZONTAL) {
    if ((xp + l) > SGUI_SCREEN_MAXX)
      l = SGUI_SCREEN_MAXX - xp;
  } else {
    if ((yp + l) > SGUI_SCREEN_MAXY)
      l = SGUI_SCREEN_MAXY - yp;
  }
...
// Ecke   = xp,yp [x=0...SGUI_SCREEN_MAXX, y=0...SGUI_SCREEN_MAXY]
//--------------------------------------------------------------
void SGUI_ScreenDrawString(uint16_t xp, uint16_t yp, char *ptr, UB_Font *font,
...
// c2 = hintergrundfarbe
//--------------------------------------------------------------
void SGUI_ScreenDrawArrowBtn(uint16_t x, uint16_t y, uint16_t s,
                             SSCREEN_ARROW_t m, uint16_t c1, uint16_t c2) {
  // check auf limit
  if (x >= SGUI_SCREEN_MAXX)
    x = SGUI_SCREEN_MAXX - 1;
  if (y >= SGUI_SCREEN_MAXY)
    y = SGUI_SCREEN_MAXY - 1;
  if (s == 0)
    return;
  if ((x + s) > SGUI_SCREEN_MAXX)
    s = SGUI_SCREEN_MAXX - x;
  if ((y + s) > SGUI_SCREEN_MAXY)
    s = SGUI_SCREEN_MAXY - y;
...
//--------------------------------------------------------------
void SGUI_ScreenDraw3DFrame(uint16_t x, uint16_t y, uint16_t w, uint16_t h,
                            STYLE_TYP_t style) {
  // check auf limit
  if (x >= SGUI_SCREEN_MAXX)
    x = SGUI_SCREEN_MAXX - 1;
  ...
  if (style == STYLE_FLAT) {
    ...
    // col3 (unten+rechts)
    SGUI_ScreenStraight(x + 1, y + h - 2, w - 2, LCD_DIR_HORIZONTAL,
                        SGUI_3DCOL_3);
    SGUI_ScreenStraight(x + w - 2, y + 1, h - 2, LCD_DIR_VERTICAL,
                        SGUI_3DCOL_3);
    // col4 (unten+rechts)
...
// style : [STYLE_RAISED, STYLE_LOWERED]
//--------------------------------------------------------------
void SGUI_ScreenDraw3DCircle(uint16_t x, uint16_t y, uint16_t r,
                             STYLE_TYP_t style) {
  // check auf limit
  if (x >= SGUI_SCREEN_MAXX)
    x = SGUI_SCREEN_MAXX - 1;
  ...
  if (r == 0)
    return;
  if ((x + r) > SGUI_SCREEN_MAXX)
    r = SGUI_SCREEN_MAXX - x;
  if ((y + r) > SGUI_SCREEN_MAXY)
    r = SGUI_SCREEN_MAXY - y;
...
// w,h = breite, hoehe
//--------------------------------------------------------------
void SGUI_ScreenDrawImage(UB_Image *img, uint16_t x, uint16_t y, uint16_t w,
                          uint16_t h) {
#if SGUI_SCREEN_ORIENTATION != 0
  DMA2D_Koord koord;
#else
