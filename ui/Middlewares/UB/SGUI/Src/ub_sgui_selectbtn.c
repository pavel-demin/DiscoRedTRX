...
SSELECTBTN_t *SGUI_SelectButtonCreate(uint16_t x, uint16_t y, uint16_t w,
  ...
  if (SGUI_WINDOW.akt_win_ptr == NULL)
    return NULL;
  if (SGUI_SELECTBTN.akt_anz >= (MAX_SELECTBTN_ANZ - 1))
    return NULL;
...
// c3 = Hintergrund
//--------------------------------------------------------------
void SGUI_SelectButtonSetColor(SSELECTBTN_t *ptr, uint16_t c1, uint16_t c2,
                               uint16_t c3) {
  if (ptr == NULL)
    return;
...
//--------------------------------------------------------------
bool SGUI_SelectButtonTouch(bool pressed, uint16_t x, uint16_t y,
                            bool blocked) {
  bool ret_wert = false;
  uint32_t n;
    ...
    if (m == 1) {
      if (SGUI_SELECTBTN.ptr[n]->typ == SSELECTBTN_H) {
        if (x > (x1 + SGUI_SELECTBTN.ptr[n]->width / 2)) {
          // pfeil rechts
          if (SGUI_SELECTBTN.ptr[n]->item_aktiv <
              SGUI_SELECTBTN.ptr[n]->item_cnt - 1) {
            SGUI_SELECTBTN.ptr[n]->item_aktiv++;
            // neu zeichnen
            P_SGUI_SelectButtonDraw(SGUI_SELECTBTN.ptr[n]);
            // CallBack aufrufen
            if (SGUI_SELECTBTN.ptr[n]->fkt_ptr != NULL)
              (*SGUI_SELECTBTN.ptr[n]->fkt_ptr)(
                  SGUI_SELECTBTN.ptr[n]->item_aktiv);
          }
        } else {
          // pfeil links
