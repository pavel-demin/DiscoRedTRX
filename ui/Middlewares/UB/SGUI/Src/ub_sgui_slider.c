...
SSLIDER_t *SGUI_SliderCreate(uint16_t x, uint16_t y, uint16_t w, uint16_t h) {
  ...
  if (SGUI_WINDOW.akt_win_ptr == NULL)
    return NULL;
  if (SGUI_SLIDER.akt_anz >= (MAX_SLIDER_ANZ - 1))
    return NULL;
...
void SGUI_SliderSetMinMax(SSLIDER_t *ptr, int32_t min, int32_t max) {
  ...
  if (ptr->akt_value < min)
    ptr->akt_value = min;
  if (ptr->akt_value > max)
    ptr->akt_value = max;
}
...
void SGUI_SliderSetValue(SSLIDER_t *ptr, int32_t value) {
  if (ptr == NULL)
    return;
  if (value < ptr->min_value)
    value = ptr->min_value;
  if (value > ptr->max_value)
    value = ptr->max_value;
      ...
      if (SGUI_SLIDER.ptr[n]->typ == SSLIDER_H) {
        if (x > (x1 + SGUI_SLIDER.ptr[n]->width / 2)) {
          ...
          if (SGUI_SLIDER.ptr[n]->akt_value < SGUI_SLIDER.ptr[n]->max_value) {
            ...
            // CallBack aufrufen
            if (SGUI_SLIDER.ptr[n]->fkt_ptr != NULL)
              (*SGUI_SLIDER.ptr[n]->fkt_ptr)(SGUI_SLIDER.ptr[n]->akt_value);
          }
        } else {
          // pfeil links
...
void P_SGUI_SliderDraw(SSLIDER_t *ptr) {
  ...
  if (ptr->typ == SSLIDER_H) {
    ...
    if ((ptr->status & SSLIDER_BIT_ARROW) != 0) {
      ...
      d2 = (w - 2 * d - 3 * btn_size);
      f = ((float)(d2) / (float)(d1)) *
          ((float)(ptr->akt_value) - (float)(ptr->min_value));
      if (f < 0)
        f = 0;
      px = (uint16_t)(f);
      SGUI_ScreenDrawFullRect(x + d + btn_size + px, btn1_yp, btn_size,
                              btn_size, c1);
      if ((ptr->status & SSLIDER_BIT_3DSTYLE) != 0) {
        SGUI_ScreenDraw3DFrame(x + d + btn_size + px, btn1_yp, btn_size,
                               btn_size, STYLE_RAISED);
      }
    } else {
      // check der abmessungen
      if (btn_size + 2 * d >= w)
      ...
      if ((ptr->status & SSLIDER_BIT_3DSTYLE) != 0) {
        SGUI_ScreenDraw3DFrame(x + d + px, btn1_yp, btn_size, btn_size,
                               STYLE_RAISED);
      }
    }
    ...
    if ((ptr->status & SSLIDER_BIT_ARROW) != 0) {
      ...
      if (f < 0)
        f = 0;
      px = (uint16_t)(f);
      SGUI_ScreenDrawFullRect(btn1_xp, y + d + btn_size + px, btn_size,
                              btn_size, c1);
      if ((ptr->status & SSLIDER_BIT_3DSTYLE) != 0) {
        SGUI_ScreenDraw3DFrame(btn1_xp, y + d + btn_size + px, btn_size,
                               btn_size, STYLE_RAISED);
      }
    } else {
      // check der abmessungen
      ...
      if ((ptr->status & SSLIDER_BIT_3DSTYLE) != 0) {
        SGUI_ScreenDraw3DFrame(btn1_xp, y + d + px, btn_size, btn_size,
                               STYLE_RAISED);
      }
    }
  }
