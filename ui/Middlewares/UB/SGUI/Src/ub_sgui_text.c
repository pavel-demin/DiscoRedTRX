...
//--------------------------------------------------------------
void SGUI_TextSetDefFont(UB_Font *font) { SGUI_TEXT.akt.font = font; }
...
void SGUI_TextPrintString(char *txt_ptr) {
  ...
  // Text zeichnen
  SGUI_ScreenDrawString(x, y, txt_ptr, SGUI_TEXT.akt.font,
                        SGUI_TEXT.akt.col_text, SGUI_TEXT.akt.col_back);
...
void SGUI_TextPrintInt(int32_t value, uint8_t digits, bool padding) {
  ...
  // Text zeichnen
  SGUI_ScreenDrawString(x, y, SGUI_WINDOW.buf, SGUI_TEXT.akt.font,
                        SGUI_TEXT.akt.col_text, SGUI_TEXT.akt.col_back);
...
STEXT_t *SGUI_TextCreateString(char *txt_ptr) {
  ...
  if (SGUI_WINDOW.aktiv_window == 0)
    return NULL;
  if (SGUI_TEXT.akt_anz >= (MAX_TEXT_ANZ - 1))
    return NULL;
  ...
  for (n = 0; n < SGUI_TEXT.akt_anz; n++) {
    ...
    if (SGUI_WINDOW.aktiv_window == SGUI_TEXT.ptr[n]->window_nr) {
      ...
      if ((status != 0) || (SGUI_WINDOW.update_all == true))
        P_SGUI_TextDraw(SGUI_TEXT.ptr[n]);
    }
  }
}
...
void P_SGUI_TextDraw(STEXT_t *ptr) {
  ...
  if (ptr->typ == STEXT_ASCII) {
    // Text zeichnen
    SGUI_ScreenDrawString(x, y, ptr->txt_ptr, ptr->font, ptr->col_text,
                          ptr->col_back);
    ...
    // Text zeichnen
    SGUI_ScreenDrawString(x, y, SGUI_WINDOW.buf, ptr->font, ptr->col_text,
                          ptr->col_back);
